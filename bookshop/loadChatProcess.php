...
$chat_rs = Connection::select("SELECT * FROM `chat` WHERE `from`='" . $_SESSION["i"]["email"] . "'");
...
if ($chat_num != 0) {
    for ($i = 0; $i < $chat_num; $i++) {
        ...
        if ($chat_data["status"] == "1" || $chat_data["status"] == "2") {
?>
            <!-- sent -->
        ...
        <?php
            ...
            Connection::iud("UPDATE `chat` SET `status`='4' WHERE `chat_id`='".$chat_data["chat_id"]."'");
        ?>
            <!-- received -->
            <div class="col-12 mt-2">
                <div class="row">
